...
const ChapterSlideBar = ({
      ...
      case "certificate":
        return (
          <div className="mt-4">
            ...
            <p className={`text-xs ${textSecondary}`}>Complete the course to earn your certificate</p>
          </div>
        );
      ...
      case "discussions":
        return (
          <div className="mt-4">
            ...
            <p className={`text-xs ${textSecondary}`}>Join the course discussion forum</p>
          </div>
        );
  ...
  return (
    <>
      ...
      <div className={`hidden md:flex fixed top-0 left-0 lg:left-20 z-40 h-[calc(100vh-4rem)] w-16 ${navBg} border-r ${borderColor} flex-col shadow-md`}>
        ...
        <button
          ...
          className={`flex items-center justify-center w-10 h-10 mx-auto mb-4 rounded-lg ${buttonBg} ${buttonHoverBg} transition ${textPrimary}`}
      ...
      {mobileMenuOpen && (
        <>
          {/* Backdrop */}
          <div
            className={`fixed inset-0 ${overlayBg} z-40 md:hidden`}
            onClick={() => setMobileMenuOpen(false)}
          />

          {/* Slide-in Panel */}
          <div className={`fixed inset-y-0 right-0 z-50 w-full sm:w-80 ${navBg} shadow-2xl transform transition-transform duration-300 md:hidden overflow-hidden flex flex-col`}>
            {/* Header */}
            <div className={`flex items-center justify-between p-3 sm:p-4 border-b ${borderColor} flex-shrink-0`}>
              <h2 className={`text-base sm:text-lg font-semibold ${textPrimary}`}>Course Menu</h2>
              <button
                onClick={() => setMobileMenuOpen(false)}
                className={`p-2 ${closeButtonBg} rounded-lg transition`}
