...
const UnattemptedMockInterviewCard = () => {
  ...
  const cardBg =
    mode === 'dark'
      ? 'bg-[#1B2E31]/80 hover:bg-[#243c3f]'
      : 'bg-gradient-to-br from-[#fafdfe] via-[#e8f4f5] to-[#def1f4] hover:from-[#f0fafa] hover:to-[#e2f6f7]';
  const cardBorder = mode === 'dark' ? 'border-gray-700' : 'border-gray-100';
  const iconColor = mode === 'dark' ? 'text-gray-400' : 'text-[#1f7370]';

  return (
    <div
      className={`mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4 py-3 max-w-6xl `}
    >
          ...
          >
            ...
            <div className={`flex items-center justify-between ${textPrimary} text-sm font-medium mb-3`}>
              <div className="flex items-center gap-1">
                <IoIosBookmarks className={`text-xl ${iconColor}`} />
            ...
            <div className="flex items-center justify-between mt-4">
              ...
              <button
                className={`px-4 py-1.5 cursor-pointer text-sm font-medium rounded-md transition-all duration-200 shadow-sm border ${
                  mode === 'dark'
                    ? 'bg-[#00B2A9] text-white border-[#00B2A9] hover:bg-transparent hover:text-[#00B2A9]'
                    : 'bg-[#2ab3af] text-white border-[#b2dfe6] hover:bg-white hover:text-[#2ab3af] hover:border-[#2ab3af]'
                }`}
              >
                Take Interview
              </button>
