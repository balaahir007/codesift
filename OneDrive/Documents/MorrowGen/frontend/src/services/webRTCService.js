...
class WebRTCService {
  ...
  async initialize(spaceId, userId, meetId) {
    try {
      ...
      console.log('🚀 Initializing WebRTC service:', { spaceId: this.spaceId, userId: this.userId, meetId: this.meetId });
      ...
      this.isVideoEnabled = false;
      this.isAudioEnabled = false;
      console.log('ℹ️ Starting meet without mic/camera. User will enable on demand.');
      ...
      this.socket.emit('join-meet-room', { meetId: this.meetId, spaceId: this.spaceId });
    ...
    if (!this.socket) return;
     ...
     this.socket.on('participant-joined', ({ userId, meetId }) => {
       ...
       if (this._shouldInitiateCall(otherId)) {
         ...
         console.log('↩️ Skipping dial (non-initiator) to avoid collision with', otherId);
     ...
     this.socket.on('webrtc-media-state', ({ userId, meetId, audioEnabled, videoEnabled }) => {
       if (String(meetId) !== String(this.meetId)) return;
    
       if (this.onConnectionStateChange) {
         const pc = this.peerConnections.get(String(userId));
         this.onConnectionStateChange(String(userId), pc?.connectionState || 'connected');
       }
     });
  ...
  connectToParticipants(participants) {
    ...
    for (const p of participants) {
      ...
      if (this._shouldInitiateCall(otherId)) {
        ...
        setTimeout(() => {
          if (!this.activeCalls.has(otherId)) {
            console.log('🛟 Non-initiator rescue dial (connectToParticipants) to', otherId);
            this._forceCall(otherId);
          }
        }, 2500);
      }
  ...
  _callUser(otherUserId) {
    ...
    if (typeof this._shouldInitiateCall === 'function' && !this._shouldInitiateCall(otherUserId)) {
      ...
      try {
        ...
        setTimeout(() => {
          try {
            ...
            const newCall = this.peer.call(String(otherId), stream || this._getEmptyStream());
  ...
  _ensureVideoSenders() {
    for (const [otherId, call] of this.activeCalls.entries()) {
      try {
        ...
        if (!hasVideoSender) {
          console.log('↻ Re-inviting', otherId, 'with updated local stream to add video sender');
          try { call.close(); } catch {}
          try {
            const newCall = this.peer.call(String(otherId), this.localStream || this._getEmptyStream());
            this._registerCall(newCall);
          } catch (e) {
  ...
  async toggleVideo() {
    ...
    if (next) {
      ...
      try {
        ...
        setTimeout(() => {
          try {
            ...
            console.error('❌ Failed to re-invite peers after video enable:', e);
          }
        }, 200);
      } catch (err) {
      ...
      try {
        ...
        if (this.cameraStream) {
          try { this.cameraStream.getTracks().forEach((tr) => tr.stop()); } catch {}
          this.cameraStream = null;
        }
      } catch {}

      // Mute outgoing senders (no camera hardware, no video frames)
      try { this._disableOutgoingVideoSenders?.(); } catch {}
    ...
    try {
      ...
      console.log('📡 Broadcasted video state:', this.isVideoEnabled ? 'ON' : 'OFF');
    } catch (error) {
  ...
  async toggleAudio() {
      ...
      try {
        ...
        setTimeout(() => {
          try {
            ...
            console.error('❌ Failed to re-invite peers after audio enable:', e);
          }
        }, 200);
      } catch (err) {
    ...
    try {
      ...
      console.log('📡 Broadcasted audio state:', this.isAudioEnabled ? 'ON' : 'OFF');
    } catch (error) {
      ...
      if (this.cameraStream) {
        ...
        if (cameraVideoTrack) {
          ...
          try {
            ...
            console.error('❌ Failed to re-invite peers after screen share stop:', e);
          }
        } else {
          ...
          try { this._reinviteAll(this.localStream); } catch (e) { console.error(e); }
        }
      } else {
  ...
  cleanup() {
    ...
    try { this.remoteCompositeStreams?.clear(); } catch {}
    this.spaceId = null;
    this.userId = null;
    this.meetId = null;
